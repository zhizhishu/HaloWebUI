				...
				<div class="flex flex-col gap-4">
					...
					<div class="inline-flex max-w-full flex-wrap items-center gap-1.5 self-start rounded-xl bg-gray-100/70 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] dark:bg-gray-850/80 dark:shadow-none @[64rem]:flex-nowrap @[64rem]:shrink-0">
						{#each allTabs as tab (tab.key)}
							<button
								type="button"
								class={`flex min-w-0 items-center justify-start gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all ${selectedTab === tab.key ? 'bg-white text-gray-900 shadow-[0_1px_3px_rgba(15,23,42,0.08)] dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:bg-white/50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800/50 dark:hover:text-gray-200'}`}
