<script lang="ts">
	...
	const setEnabledFilter = (value: string) => {
		enabledFilter = value as EnabledFilter;
	};
	const setHiddenFilter = (value: string) => {
		hiddenFilter = value as HiddenFilter;
	};
	const setVisibilityFilter = (value: string) => {
		visibilityFilter = value as VisibilityFilter;
	};
	...
	> = {
		all: {
			label: '全部模型',
			description: '查看和管理所有已连接的模型。',
			badgeColor: 'bg-gray-100 dark:bg-gray-800/40',
			iconColor: 'text-gray-500 dark:text-gray-400'
		},
		openai: {
			label: 'OpenAI 接口',
			description: '管理通过 OpenAI 兼容接口连接的模型。',
			badgeColor: 'bg-emerald-50 dark:bg-emerald-950/30',
			iconColor: 'text-emerald-500 dark:text-emerald-400'
		},
		gemini: {
			label: 'Gemini 接口',
			description: '管理通过 Google Gemini 接口连接的模型。',
		...
		ollama: {
			label: 'Ollama 接口',
			description: '管理通过 Ollama 本地接口连接的模型。',
			badgeColor: 'bg-violet-50 dark:bg-violet-950/30',
			iconColor: 'text-violet-500 dark:text-violet-400'
		},
		other: {
			label: '其他接口',
			description: '管理未归类到上述接口的模型。',
	...
	let groupSortable: ReturnType<typeof Sortable.create> | null = null;
	let groupStats: Map<
		string,
		{
			count: number;
	...
	const cleanIdentityValue = (value: any) =>
		value === null || value === undefined ? '' : `${value}`.trim();
	const getExplicitModelSelectionKey = (model: any) =>
		cleanIdentityValue(
			model?.selection_id ?? model?.selectionId ?? model?.selection_key ?? model?.selectionKey
		);
	const getModelCleanIdForList = (model: any) =>
		cleanIdentityValue(model?.model_id) ||
		cleanIdentityValue(model?.original_id) ||
		cleanIdentityValue(model?.originalId) ||
		cleanIdentityValue(model?.id);
	const getModelDedupKey = (model: any) => {
		const modelRef =
			model?.model_ref && typeof model.model_ref === 'object' ? model.model_ref : null;
		const selectionKey = getExplicitModelSelectionKey(model);
		const cleanId = getModelCleanIdForList(model);
		if (selectionKey) return `selection\0${selectionKey}`;

		if (modelRef) {
			return [
				'ref',
				cleanIdentityValue(modelRef.provider).toLowerCase(),
				cleanIdentityValue(modelRef.source),
				cleanIdentityValue(modelRef.connection_id ?? modelRef.prefix_id),
				cleanIdentityValue(modelRef.connection_index),
				cleanId
			].join('\0');
		}
		...
		return [
			'legacy',
			cleanIdentityValue(model?.owned_by).toLowerCase(),
			cleanIdentityValue(model?.connection_name),
			cleanIdentityValue(model?.connection_id ?? model?.prefix_id),
			cleanIdentityValue(model?.connection_index ?? model?.urlIdx),
			cleanId
		].join('\0');
	};
	const deduplicateModelsForList = (items: any[] = []) => {
		const seenRowKeys = new Set<string>();
		const seenSelectionKeys = new Set<string>();
		const result: any[] = [];

		for (const model of items ?? []) {
			const rowKey = getModelDedupKey(model);
			const selectionKey = getExplicitModelSelectionKey(model);
			if (
				(rowKey && seenRowKeys.has(rowKey)) ||
				(selectionKey && seenSelectionKeys.has(selectionKey))
			) {
				continue;
			}
	...
	const init = async () => {
		...
		try {
			...
			if (baseModelsResult.status === 'fulfilled') {
				baseModels = (baseModelsResult.value ?? []) as any[];
			} else {
			...
			const uniqueBaseModels = deduplicateModelsForList(baseModels ?? []);
			const baseModelLookup = buildModelIdentityLookup(uniqueBaseModels);
			let nextModels =
				uniqueBaseModels.map((m: any) => {
	...
	const initExpandedGroups = () => {
		...
		// Start collapsed by default; search still auto-opens matching groups below.
		expandedGroups = {};
	...
	const bulkApplyPatch = async (patch: any) => {
		...
		try {
			...
			const res = (await bulkUpsertBaseModels(localStorage.token, items, patch)) as any;
	...
	const setModelActive = async (m: any, nextActive: boolean) => {
		...
		try {
			await bulkUpsertBaseModels(
				localStorage.token,
				[{ id: modelKey, name: m?.name ?? modelKey }],
				{
	...
	const initGroupSortable = () => {
		...
		groupSortable = Sortable.create(groupListEl, {
			...
			onEnd: () => {
				...
				const newOrder = Array.from(items).map((el) => (el as HTMLElement).dataset.groupKey!);
		...
		<div class="h-full space-y-6 overflow-y-auto scrollbar-hidden">
			<div class="max-w-6xl mx-auto space-y-6">
				...
				<section class="glass-section p-4 space-y-3">
					...
					{#if filterOpen}
						<div
							class="rounded-xl border border-gray-200/60 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/30 p-3 space-y-2.5"
						>
							<!-- Status -->
							<div class="flex items-center gap-2.5">
								...
								<div class="flex flex-wrap gap-1">
									{#each [{ value: 'all', label: $i18n.t('All') }, { value: 'enabled', label: $i18n.t('Enabled') }, { value: 'disabled', label: $i18n.t('Disabled') }] as opt}
										<button
							...
							<div class="flex items-center gap-2.5">
								...
								>
								<div class="flex flex-wrap gap-1">
									{#each [{ value: 'all', label: $i18n.t('All') }, { value: 'public', label: $i18n.t('Public') }, { value: 'private', label: $i18n.t('Private') }, { value: 'unset', label: $i18n.t('Default') }] as opt}
										<button
				...
				<section class="glass-section p-4">
					...
					{#if listLoadError}
						<div
							class="mb-3 rounded-xl border border-amber-200/70 bg-amber-50/80 px-3 py-2 text-xs text-amber-700 dark:border-amber-900/40 dark:bg-amber-900/15 dark:text-amber-300"
						>
							{listLoadError}
						</div>
					{/if}
									...
									<div class="group-sortable-item" data-group-key={key}>
										<div class="px-2 py-1.5">
											<div
												...
												tabindex="0"
												on:keydown={(e) => {
													if (e.key === 'Enter' || e.key === ' ') toggleGroupExpanded(key);
												}}
											>
												<div class="flex items-center gap-2 min-w-0">
													...
													{#if selectMode}
														...
														<input
															...
															checked={selectedInGroup === (stats?.count ?? 0) &&
																(stats?.count ?? 0) > 0}
												...
												<div
													class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 shrink-0"
												>
													{#if (stats?.enabledCount ?? 0) < (stats?.count ?? 0)}
														...
														>
															{(stats?.count ?? 0) - (stats?.enabledCount ?? 0)}
															{$i18n.t('Disabled')}
														</span>
													{/if}
