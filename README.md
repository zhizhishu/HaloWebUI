...
- 修复新开聊天时发送按钮可能显示 Stop、点击无效或消息卡在边框内的问题。
- `custom` 分支新增聊天响应状态判断，只把未完成 assistant 回复或真实后台 task 视为生成中。
- 当前最后一条消息是用户消息时，不再误判为正在生成，发送按钮可以继续发起新请求。
- 这条修复和旧聊天工具过滤一起保护新旧会话发送链路，避免 UI 状态和真实请求状态不一致。

### 配置刷新鉴权修复

- 修复已登录页面刷新 `/api/config` 时未显式携带 token 的问题。
- 当浏览器 cookie 丢失但 `localStorage.token` 仍有效时，设置页不会再误拿未登录精简配置。
- 这能减少接口配置“页面看不到、运行时仍能调用”的错位现象。
...
```bash
uv run pytest <目标后端回归集合> -q
# 资源继承、MCP/工具权限、联网搜索和聊天发送相关后端回归通过，最近一次为 63 passed

npx vitest run <目标前端回归集合>
# 配置刷新鉴权、旧聊天工具过滤、资源继承、模型能力和新建对话发送状态回归通过

NODE_OPTIONS=--max-old-space-size=4096 npm run build
# production build passed
```

远端 `custom` 与 `future` 分支的 `Custom Regression Guard` 与 Docker 镜像构建也已通过。以上验证说明“可点击、可保存、模型可用、MCP 可用、指定为空不越权继承、接口配置刷新不降级、旧聊天删除工具后仍可继续对话、新建对话发送按钮不再误卡 Stop”这条核心链路未发现回归；服务器仍建议拉取最新镜像后在真实账号界面做一次人工确认。
...
- 新旧聊天发送链路都有状态自修复和回归覆盖，减少按钮卡住、旧工具残留和请求状态漂移。
...
- 已登录配置刷新会自动带上本地 token，避免 cookie 缺失时拿到未登录版配置。
...
| 新建对话发送 | 新聊天 UI 状态误判导致发送按钮卡住或显示 Stop | 已修复响应状态判断 |
| 配置一致性 | 页面看不到但运行时还能调用 | 已修复 provider 配置读写路径与 `/api/config` 鉴权刷新 |
