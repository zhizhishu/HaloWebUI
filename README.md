...
```bash
uv run pytest backend/open_webui/test/unit/test_resource_inheritance_options.py backend/open_webui/test/unit/test_models_sharing.py backend/open_webui/test/unit/test_user_tools_mcp_inherit.py -q
# 23 passed

npx vitest run src/lib/utils/resource-inheritance.test.ts src/lib/utils/model-capabilities.test.ts
# 8 passed
...
远端 `custom` 分支的 `Custom Regression Guard` 与 Docker 镜像构建也已通过。以上验证说明“可点击、可保存、模型可用、MCP 可用、指定为空不越权继承”这条核心链路未发现回归；服务器仍建议拉取最新镜像后在真实账号界面做一次人工确认。

## 核心能力

- 多供应商接口: OpenAI 兼容接口、Anthropic Claude、Google Gemini、xAI Grok、Ollama 等。
- 管理员集中配置: 管理员维护模型和 MCP，普通用户按授权继承。
- 精细用户授权: 每个用户都可以分别控制模型继承和 MCP 继承范围。
- MCP 工具集成: 支持服务端 stdio MCP，适合私有化工具链接入。
- 私有化部署: 支持 Docker、Docker Compose、PostgreSQL、Redis 和反向代理部署。
- 上游同步策略: `main` 跟随 Open WebUI，`custom` 保留本仓库创新。
