...
async def get_task_config(request: Request, user=Depends(get_verified_user)):
    return _serialize_task_config(request)
...
async def update_task_config(
    ...
    for field, value in updates.items():
        if value is None and field not in TASK_CONFIG_NULLABLE_FIELDS:
            continue

        setattr(request.app.state.config, field, value)
...
async def generate_follow_ups(
    ...
    payload = {
        ...
        **(
            {"max_tokens": 256}
            if models[task_model_id].get("owned_by") == "ollama"
            else {"max_completion_tokens": 256}
        ),
        "metadata": {
            **(request.state.metadata if hasattr(request.state, "metadata") else {}),
    ...
    try:
        ...
        log.error(f"Error generating follow-ups: {e}")
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"detail": str(e)},
        )
