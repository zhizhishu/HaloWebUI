...
from open_webui.routers.tools import enrich_schema_with_dynamic_options
...
async def toggle_global_by_id(request: Request, id: str, user=Depends(get_admin_user)):
    function = Functions.get_function_by_id(id)
    if function:
        function = Functions.update_function_by_id(
...
async def get_function_valves_spec_by_id(
    ...
    if function:
        ...
        if hasattr(function_module, "Valves"):
            ...
            return enrich_schema_with_dynamic_options(function_module, schema)
        return None
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
