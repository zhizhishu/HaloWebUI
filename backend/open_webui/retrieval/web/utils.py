...
def _get_playwright_loader_class():
...
class SafeFireCrawlLoader(BaseLoader, RateLimitMixin, URLProcessingMixin):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        firecrawl_loader_cls = _get_firecrawl_loader_class()
        for url in self.web_paths:
            try:
                self._safe_process_url_sync(url)
                loader = firecrawl_loader_cls(
                    url=url,
...
class SafePlaywrightURLLoader(BaseLoader, RateLimitMixin, URLProcessingMixin):
    ...
    def __init__(
        ...
        loader_cls = _get_playwright_loader_class()

        # We'll set headless to False if using playwright_ws_url since it's handled by the remote browser
        self._delegate = loader_cls(
            urls=web_paths,
        ...
        self.urls = self._delegate.urls
        self.continue_on_failure = self._delegate.continue_on_failure
        self.headless = self._delegate.headless
        self.proxy = self._delegate.proxy
        self.evaluator = self._delegate.evaluator
        self.verify_ssl = verify_ssl
        self.requests_per_second = requests_per_second
        self.last_request_time = None
