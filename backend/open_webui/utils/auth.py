...
# Simple TTL cache for authenticated user lookups to reduce DB queries
_user_cache: dict[str, tuple[float, any]] = {}
_USER_CACHE_TTL = 5  # seconds


def _get_cached_user(user_id: str):
    """Get user from cache if still fresh, else return None."""
...
def create_token(data: dict, expires_delta: Union[timedelta, None] = None) -> str:
    ...
    # Add a unique session ID (jti) for multi-device session tracking
...
def get_current_user(
    ...
    if data is not None and "id" in data:
        ...
        # One-time/conditional migration for per-user external connections.
        # Safe: only writes when the legacy/global config needs to be copied into user settings.
        user = maybe_migrate_user_connections(request, user)
        return user
    else:
        raise HTTPException(
