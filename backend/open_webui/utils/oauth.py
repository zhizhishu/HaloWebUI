...
from open_webui.utils.encryption import encrypt_token
import uuid
...
# Lock to prevent race conditions during OAuth user creation
_oauth_user_lock = asyncio.Lock()
...
class OAuthManager:
    ...
    async def handle_callback(self, request, provider, response):
        ...
        # Check if the user exists (under lock to prevent race conditions)
        async with _oauth_user_lock:
            ...
            if user:
                ...
                # Sync profile fields on login if configured
                profile_updates = {}
                if OAUTH_UPDATE_NAME_ON_LOGIN:
                    username_claim = auth_manager_config.OAUTH_USERNAME_CLAIM
                    oauth_name = user_data.get(username_claim)
                    if oauth_name and oauth_name != user.name:
                        profile_updates["name"] = oauth_name
                if OAUTH_UPDATE_EMAIL_ON_LOGIN:
        ...
        if ENABLE_OAUTH_SIGNUP.value:
            ...
            if oauth_id_token:
                encrypted = encrypt_token(oauth_id_token)
            else:
                encrypted = ""
            response.set_cookie(
                key="oauth_id_token",
